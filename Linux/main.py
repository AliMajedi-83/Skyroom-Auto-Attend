...
class SkyroomGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Skyroom Auto-Attend Bot")
        self.root.geometry("950x800")

        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        app_logger.info("Initializing Skyroom GUI application.")
        database.init_db()
    ...
    def on_closing(self):
        ...
        if active_running:
            msg = "A class session is currently active.\nIf you exit, Firefox will close and the video will be saved in the background terminal.\n\nAre you sure you want to exit?"
            if not messagebox.askyesno("Confirm Exit", msg):
                return
                
        app_logger.info("GUI closed by user. Stopping all active bots...")
    ...
    def build_add_tab(self):
        ...
        self.ent_silence.insert(0, "15")  # تغییر مقدار پیش‌فرض به 15 تا تداخل ایجاد نشود
    ...
    def toggle_ignore_next(self):
        ...
        new_state = not current_state  # معکوس کردن وضعیت
        ...
        self.refresh_list()
        self.on_tree_select(None) # آپدیت رنگ دکمه
        ...
        if messagebox.askyesno("Join Now", f"Start '{target[3]}' immediately?"):
            ...
            for active_bot in self.active_bots:
                if active_bot.is_running:
                    app_logger.info("Terminating previous class session to prevent overlap before manual join.")
                    active_bot.stop_all()
            ...
            for cls in classes:
                try:
                    ...
                    if current_day in schedule_dict and schedule_dict[current_day] == current_time:
                        ...
                        for active_bot in self.active_bots:
                            if active_bot.is_running:
                                app_logger.info(f"Terminating previous class session to prevent overlap.")
                                active_bot.stop_all()
